...
Approximate targeted core `MS-XAML-2017` support: **94%**. This estimate covers the scoped language/object-mapping target in this matrix, not full WPF vocabulary parity.

Next slice:

1. Add preserved-only coverage for the remaining declaration/schema intrinsics such as `x:Subclass`, `x:Members`, and `x:Property`.
2. Keep arbitrary CLR execution deferred until the parser/validator can represent the full directive and declaration surface safely.
