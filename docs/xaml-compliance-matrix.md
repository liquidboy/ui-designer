...
| Namespace declarations | Current | Current | Preserve prefix, namespace URI, and declaration scope. | Known namespaces validate against active registries, and lexical namespace scope now resolves type references such as `{x:Type ui:TextBlock}`, declaration `Type="ui:TextBlock"`, validation-only `clr-namespace:System` primitive aliases, and configured no-execution `Liquidboy.UI.Designer*` CLR namespace mappings. |
...
| Object element syntax | Current | Current | Convert XML elements to object nodes when schema says they are types. | Active vocabularies now cover runtime and designer config types, including configured no-execution CLR namespace aliases such as `clr-namespace:Liquidboy.UI.Designer;assembly=ui-designer`. |
...
| `x:Class` | Current | Phase 4 | Parse, preserve, require a non-empty CLR-style qualified type name, and enforce root-only placement. | Preserved-only; no markup compilation. | Non-root usage now raises `invalid-directive-placement`, and malformed class names raise `invalid-clr-type-name`. |
| `x:Uid` | Current | Phase 4 | Parse and preserve with warning. | Preserved-only. | Useful for localization metadata, not rendering. |
| `x:TypeArguments` | Partial | Phase 4 | Parse, preserve with warning, and validate comma-separated/nested type-name lists against in-scope namespaces, including `clr-namespace:System` primitive aliases and configured no-execution CLR namespace mappings. | Preserved-only. | Generic type execution remains unsupported. |
| `x:FactoryMethod`, `x:Arguments`, `x:ConstructorArgs`, `x:InitializationText` | Partial | Construction directive support | Parse, preserve, validate `x:FactoryMethod` as a CLR identifier, validate placement for construction argument property elements, reject empty construction argument lists, reject text-only argument lists, and prevent duplicate `x:Arguments`/`x:ConstructorArgs` forms on the same object. | Preserved-only with warnings; no constructor or factory execution. | `x:Arguments` is accepted as the XAML 2009 form, while `x:ConstructorArgs` covers the MS-XAML schema-style construction argument name. |
| `x:ClassModifier`, `x:FieldModifier`, `x:Code`, `x:XData` | Partial | Markup-compilation/XML data metadata support | Parse, preserve, validate supported class/field modifier values, validate markup-compilation dependencies, require `x:Code` on the document root with `x:Class`, and preserve raw `x:XData` XML islands without validating island elements as UI objects. | Preserved-only with warnings; no code execution, type generation, or XML data provider execution. | `x:ClassModifier` accepts public/not-public class access tokens; `x:FieldModifier` accepts common CLR field access tokens and requires root `x:Class` plus same-object `x:Name`; `x:XData` is accepted only as a member value in the scoped subset. |
| `x:Subclass`, `x:Members`, `x:Member`, `x:Property` | Partial | Declaration/schema intrinsic support | Parse, preserve, require `x:Subclass` and `x:Members` on the root object with `x:Class`, validate `x:Subclass` as a CLR-style qualified type name, require `x:Members` property-element syntax, allow only `x:Member`/`x:Property` declaration objects, validate declaration `Name` values as CLR identifiers, require `Name`/`Type` metadata on each declaration, and validate declaration `Type` values against in-scope namespaces. | Preserved-only with warnings; no CLR member generation or type execution. | Declaration `Type` values resolve namespace-qualified UI types, supported XAML primitive types, validation-only `clr-namespace:System` primitive aliases, and configured no-execution `Liquidboy.UI.Designer*` CLR namespace aliases without loading arbitrary CLR types. |
| `x:Type` / `x:TypeExtension` | Partial | Phase 5 | Parse structured type markup extensions and object elements, validate required type names, known simple object type names, namespace-qualified UI type references, supported XAML/`clr-namespace:System` primitive type names, and configured no-execution CLR namespace aliases. | Runtime lowering evaluates supported `{x:Type ...}` and `<x:Type TypeName="..." />` forms to type-name strings; authoring serialization preserves source structure. | Arbitrary CLR type loading, generic type execution, and CLR static value resolution remain deferred. |
...
22. Type-name validation for `x:Type` and `x:Array Type` now uses in-scope XML namespace declarations, including prefix-qualified `ui-designer` types, validation-only `clr-namespace:System` primitive aliases such as `sys:Int32`, configured no-execution CLR schema aliases, and deterministic errors for unknown prefixes, CLR namespaces, assemblies, or CLR type names.
...
30. Declaration `Type` metadata on `x:Member` and `x:Property` now validates through the same namespace-aware type-name resolver used by `x:Type`, `x:Array Type`, and `x:TypeArguments`, including local property-element/declaration-object namespace mappings and deterministic unknown-type errors.
