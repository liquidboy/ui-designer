...
Approximate targeted core `MS-XAML-2017` support: **90%**. This estimate covers the scoped language/object-mapping target in this matrix, not full WPF vocabulary parity.

Next slice:

1. Add validation-only parsing/resolution for `x:TypeArguments` type-name lists, including namespace-qualified and CLR primitive aliases.
