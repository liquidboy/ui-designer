...
Approximate targeted core `MS-XAML-2017` support: **98%**. This estimate covers the scoped language/object-mapping target in this matrix, not full WPF vocabulary parity.

Next slice:

1. Validate markup-compilation modifier directive values for `x:ClassModifier` and `x:FieldModifier`.
2. Keep code generation deferred while catching unsupported modifier metadata earlier.
