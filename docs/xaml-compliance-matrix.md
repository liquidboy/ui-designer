...
Approximate targeted core `MS-XAML-2017` support: **97%**. This estimate covers the scoped language/object-mapping target in this matrix, not full WPF vocabulary parity.

Next slice:

1. Add lexical validation for CLR identifier metadata such as `x:Class`, `x:Subclass`, and construction/member declaration names.
2. Keep markup compilation and CLR generation deferred while catching malformed metadata earlier.
