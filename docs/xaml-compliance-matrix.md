...
Approximate targeted core `MS-XAML-2017` support: **89%**. This estimate covers the scoped language/object-mapping target in this matrix, not full WPF vocabulary parity.

Next slice:

1. Add validation-only `x:Static` member resolution for known schema types and supported CLR primitive constants.
