...
Approximate targeted core `MS-XAML-2017` support: **96%**. This estimate covers the scoped language/object-mapping target in this matrix, not full WPF vocabulary parity.

Next slice:

1. Add a no-execution schema mapping path for selected non-System CLR namespace type tokens.
2. Keep arbitrary CLR execution, reflection, and assembly loading deferred while still allowing configured schemas to validate declaration metadata.
