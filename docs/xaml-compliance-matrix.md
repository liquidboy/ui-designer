...
| `x:Array` | Partial | Phase 5 | Parse and validate intrinsic `x:Array` object elements, required `Type`, direct content, `x:Array.Items`, namespace-qualified `{x:Type ...}` item-type expressions, and supported primitive item types such as `x:String`, `x:Int32`, `x:Boolean`, `System.Int32`, and `sys:Int32` when `sys` maps to `clr-namespace:System`. | Authoring lowering preserves structural `Array` compatibility nodes; runtime lowering evaluates single member values and keyed resources to JavaScript arrays, coercing supported primitive item objects/text to JS primitives with range-aware integer and `Single` checks. | Arbitrary CLR array type execution, decimal precision enforcement, and generic array type execution remain deferred. |
...
Approximate targeted core `MS-XAML-2017` support: **88%**. This estimate covers the scoped language/object-mapping target in this matrix, not full WPF vocabulary parity.

Next slice:

1. Add decimal precision/range fidelity for `x:Decimal` and document the remaining numeric edge cases.
