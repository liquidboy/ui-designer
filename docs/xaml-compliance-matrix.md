...
Approximate targeted core `MS-XAML-2017` support: **93%**. This estimate covers the scoped language/object-mapping target in this matrix, not full WPF vocabulary parity.

Next slice:

1. Add preserved-only coverage for the remaining metadata/code-adjacent intrinsic directives such as `x:ClassModifier`, `x:FieldModifier`, `x:Code`, and `x:XData`.
2. Keep arbitrary CLR execution deferred until the parser/validator can represent the full directive surface safely.
