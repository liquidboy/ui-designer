...
- Intrinsic directive/type validation for root and `ResourceDictionary` namescopes, root-only `x:Class`, dictionary `x:Key`, structural `x:Array`, `{x:Type ...}`, `{x:Static ...}`, object-identity `{x:Reference ...}`, attribute/property-element `Binding`/`x:Null`, and scoped primitive/object `StaticResource` plus `DynamicResource` lookup
