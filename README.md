...
- Intrinsic directive/type validation for root, `ResourceDictionary`, template, and object-island namescopes, namespace-aware `x:Type`/`x:Array`/`x:TypeArguments` type references with validation-only `clr-namespace:System` primitive aliases, preserved construction and markup-compilation directives, raw `x:XData` islands, validation-only `x:Static` member resolution, schema-owned text syntax conversion including decimal-specific validation, root-only `x:Class`, dictionary `x:Key`, runtime array-valued `x:Array` member/resource values with range-aware primitive item coercion, object-element and markup-extension `x:Null`/`x:Type`/`x:Static`/`x:Reference`, attribute/property-element `Binding`, and scoped primitive/object `StaticResource` plus `DynamicResource` lookup
