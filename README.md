...
- Intrinsic directive/type validation for root and `ResourceDictionary` namescopes, schema-owned text syntax conversion, root-only `x:Class`, dictionary `x:Key`, runtime array-valued `x:Array` member/resource values, object-element and markup-extension `x:Null`/`x:Type`/`x:Static`/`x:Reference`, attribute/property-element `Binding`, and scoped primitive/object `StaticResource` plus `DynamicResource` lookup
