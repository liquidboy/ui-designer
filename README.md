...
- Intrinsic directive/type validation for namescopes, root-only `x:Class`, dictionary `x:Key`, structural `x:Array`, `{x:Type ...}`, `{x:Static ...}`, attribute/property-element `Binding`/`x:Null`, and scoped primitive/object `StaticResource` plus `DynamicResource` lookup
